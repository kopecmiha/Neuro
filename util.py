...
def post_process_generator_output(generator_output):
    ...
    scaled_image = generator_output * 180
    scaled_image = np.clip(scaled_image, 0, 255)
    return scaled_image


def load_inversion_stats(data):
    with torch.no_grad():
        gt_cov_inv = torch.tensor(data['cov']).to("cpu")
        mask = torch.eye(*gt_cov_inv.size()).to("cpu")
        gt_cov_inv = torch.linalg.inv(gt_cov_inv * mask).float()
        return gt_cov_inv